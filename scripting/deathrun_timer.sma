...
new Player[PlayerData], mapname[32]
...
public GameWin(id){
	...
	if (!Player[rank]){
		client_print(id, print_chat, "Congratulations on your first record on this map.")
		...
		if (Milliseconds < Player[record]){
			...
			client_print(id, print_chat, "New record (-%s improvement)", Clock(Player[record] - Milliseconds))
		}

		else if (Milliseconds == Player[record])
			client_print(id, print_chat, "You tie with your own record")

		else
			client_print(id, print_chat, "Your best record is %s", Clock(Player[record]))
	}

	client_print(0, print_chat, "%n won the round within %s", id, Clock(Milliseconds))
} 
