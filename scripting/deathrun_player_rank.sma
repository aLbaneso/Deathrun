...
public ReloadRank(){
	...
	//get_players_ex(players, num, GetPlayers_ExcludeBots|GetPlayers_ExcludeHLTV|GetPlayers_MatchTeam, "CT")
...
public clcmd_rank(id){
	if (Player[id][rank]){
		client_print(id, print_chat, "You're ranked #%d with %s", Player[id][rank], Clock(Player[id][record]))
		client_print(id, print_chat, "This record was set on %s", Date(Player[id][timestamp]))
	}
